...
def generate_routefile(seed):
    ...
    with open("working/route_gen_7.rou.xml","w") as routes:
        print("""<routes>
...
class CustomEnv(gym.Env):
    def __init__(self):
        ...
        self.action_space = spaces.MultiDiscrete([2, 2, 2, 2])  # Red and green phases only
    ...
    def step(self, action):
        try:
            num_traffic_lights = len(traci.trafficlight.getIDList())
            for idx, action_for_light in enumerate(action[:num_traffic_lights]): 
                tl_id = traci.trafficlight.getIDList()[idx] 
                traci.trafficlight.setPhase(tl_id, action_for_light)  
            traci.simulationStep()
            self.step_counter += 1
    ...
    def _get_observation(self):
        try:  # Begin try block
            ...
            observation = np.concatenate((waiting_times, densities)) 
            return observation
    ...
    def _is_done(self):
        ...
        done_flag = self.step_counter >= 10000
