...
def generate_routefile(seed):
    ...
    with open("working/route_gen.rou.xml","w") as routes:
        print("""<routes>
...
def get_state():
    ...
    try:
        for tls_id in traci.trafficlight.getIDList():
            ...
            # Traffic Speed
            average_speed = sum(traci.lane.getLastStepMeanSpeed(lane) for lane in lanes) / len(lanes)
            state.append(average_speed)

            vehicles_density = sum(traci.lane.getLastStepVehicleNumber(lane) for lane in lanes)
            state.append(vehicles_density)
...
def get_average_queue_length():
    # Get the list of lanes controlled by traffic lights
...
if __name__ == "__main__":
    ...
    # Environment setup
    state_size = 3  # Example state size
    ...
    for episode in range(1,EPISODES+1):
        ...
        try:
            ...
            traci.start(["sumo-gui", "-c", "working/fukk_1.sumocfg", "--no-warnings", "--quit-on-end"])
